...
export const initDB = () => {
    return new Promise((resolve, reject) => {
        ...
        request.onupgradeneeded = (event) => {
            ...
            // Writing Materials (Missing Store)
...
export const saveWritingMaterial = async (material) => {
    const db = await initDB();
    const tx = db.transaction('writing_materials', 'readwrite');
    const store = tx.objectStore('writing_materials');
    return new Promise((resolve, reject) => {
        const request = store.put({ ...material, updatedAt: Date.now() });
        request.onsuccess = () => resolve(material);
        request.onerror = () => reject(request.error);
    });
};

export const getWritingMaterials = async () => {
    const db = await initDB();
    const tx = db.transaction('writing_materials', 'readonly');
    const store = tx.objectStore('writing_materials');
    return new Promise((resolve, reject) => {
...
export const deleteWritingMaterial = async (id) => {
    const db = await initDB();
    const tx = db.transaction('writing_materials', 'readwrite');
    const store = tx.objectStore('writing_materials');
    return new Promise((resolve, reject) => {
