...
import {
    ...
    ArrowLeft, MoreVertical, LayoutGrid, ListFilter, FileText, Check
} from 'lucide-react';
    ...
    const renderNoteCard = (note) => (
        ...
        >
            ...
            {note.folder && note.folder !== 'Uncategorized' && (
                <div className="absolute right-4 top-5 bg-phy-glass px-2 py-0.5 rounded-full text-[10px] text-phy-muted border border-phy-border">
                    {note.folder}
                </div>
            )}
        </div>
                        ...
                        <div className="w-full max-w-lg bg-white dark:bg-gray-900 rounded-t-3xl p-6 animate-in slide-in-from-bottom flex flex-col gap-4 shadow-2xl" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                ...
                                <div className="p-4 rounded-2xl bg-phy-glass flex flex-col gap-2">
                                    <span className="text-[10px] font-bold text-phy-muted px-1">MOVE TO FOLDER</span>
                                    <select 
                                        value={note?.folder || 'Uncategorized'}
                                        onChange={(e) => handleUpdate({ folder: e.target.value })}
                                        className="w-full bg-white dark:bg-black/20 border border-phy-border rounded-xl p-2 outline-none text-sm"
                                    >
            ...
            <div className="h-full flex flex-col bg-phy-bg px-5 pt-10 pb-20 overscroll-none animate-in fade-in duration-500">
                ...
                <div className="flex items-center justify-between mb-2">
                    <div className="flex flex-col">
                        <div className="flex items-center gap-2" onClick={() => setShowFoldersMobile(!showFoldersMobile)}>
                             <h1 className="text-4xl font-bold flex items-center gap-2">
                                {activeFolder === 'All' ? '全部笔记' : activeFolder} <ChevronDown size={28} className="text-phy-muted/40" />
                             </h1>
                        </div>
                ...
                {/* Mobile Folder Selection Overlay */}
                {showFoldersMobile && (
                    <div className="fixed inset-0 z-[120] flex items-end justify-center bg-black/40 backdrop-blur-sm" onClick={() => setShowFoldersMobile(false)}>
                        <div className="w-full max-w-lg bg-white dark:bg-gray-900 rounded-t-3xl p-6 animate-in slide-in-from-bottom flex flex-col gap-4 shadow-2xl" onClick={e => e.stopPropagation()}>
                            <div className="w-12 h-1.5 bg-phy-muted/30 rounded-full mx-auto mb-2" />
                            <h3 className="text-sm font-bold text-phy-muted uppercase tracking-widest px-2">选择文件夹</h3>
                            <div className="grid grid-cols-1 gap-1 overflow-y-auto max-h-[50vh]">
                                <button 
                                    onClick={() => { setActiveFolder('All'); setShowFoldersMobile(false); }}
                                    className={`flex items-center justify-between p-4 rounded-2xl transition-colors ${activeFolder === 'All' ? 'bg-phy-accentGlass text-phy-accent' : 'hover:bg-phy-glass'}`}
                                >
                                    <span className="font-bold">全部笔记</span>
                                    {activeFolder === 'All' && <Check size={20} />}
                                </button>
                                {folders.map(f => (
                                    <button 
                                        key={f}
                                        onClick={() => { setActiveFolder(f); setShowFoldersMobile(false); }}
                                        className={`flex items-center justify-between p-4 rounded-2xl transition-colors ${activeFolder === f ? 'bg-phy-accentGlass text-phy-accent' : 'hover:bg-phy-glass'}`}
                                    >
                                        <span className="font-bold">{f}</span>
                                        {activeFolder === f && <Check size={20} />}
                                    </button>
                                ))}
                            </div>
                        </div>
        ...
        <div className="h-full min-h-0 flex animate-in fade-in bg-transparent overflow-hidden overscroll-none relative">
            ...
            <div className={`${showList ? 'w-80 border-r border-phy-border' : 'w-0 opacity-0'} h-full flex flex-col transition-all duration-500 glass-sidebar z-40 relative shrink-0 overflow-hidden`}>
                <div className="p-5 flex items-center justify-between">
                     ...
                     <div className="flex gap-2">
                        <button onClick={handleCreateFolder} className="p-2 hover:bg-phy-glassHover rounded-lg text-phy-muted transition-colors"><FolderPlus size={18} /></button>
                ...
                <div className="px-3 pb-4 flex flex-col gap-1 overflow-y-auto max-h-[200px] custom-scrollbar">
                    <div 
                        onClick={() => setActiveFolder('All')}
                        className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold cursor-pointer transition-all ${activeFolder === 'All' ? 'bg-phy-accentGlass text-phy-accent border border-phy-accent/20' : 'text-phy-muted hover:bg-phy-glassHover border border-transparent'}`}
                    >
                        <Folder size={14} className={activeFolder === 'All' ? 'text-phy-accent' : 'text-phy-muted'} />
                    ...
                    {folders.map(folder => (
                        <div 
                            ...
                            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold cursor-pointer transition-all ${activeFolder === folder ? 'bg-phy-accentGlass text-phy-accent border border-phy-accent/20' : 'text-phy-muted hover:bg-phy-glassHover border border-transparent'}`}
                        >
                            <Folder size={14} className={activeFolder === folder ? 'text-phy-accent' : 'text-phy-muted'} />
                            <span className="flex-1 truncate">{folder}</span>
                            <span className="opacity-40 text-[10px] font-medium">{notes.filter(n => (n.folder || 'Uncategorized') === folder).length}</span>
                        </div>
                    ))}
                </div>
